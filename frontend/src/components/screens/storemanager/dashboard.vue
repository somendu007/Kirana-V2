<template>
  <div class="dashboard">
    <h1>Hey {{ user.name }}</h1>
    <div
      class="card user-details bg-body-secondary"
      ...
      <div class="d-flex pt-2" style="justify-content: center">
        <div v-if="image != null" class="profile-img-container">
          <img
            ...
            class="image img-thumbnail me-2"
          />
        </div>
    ...
    <div class="card management-data">
      <div class="card-header text-start">Management Data</div>
      <div class="card-body">
        <h5 class="card-title text-start">
          Total Revenue(Month)
          <strong class="float-end">$ {{ managementData?.revenue }}</strong>
        </h5>
        <hr />
        ...
        <div class="row">
          <div class="col">
            ...
            <table class="table table-hover border-1 border">
              ...
              <tbody>
                <tr
                  v-for="section in managementData?.sections"
                  :key="section.id"
                >
          ...
          <div class="col">
            ...
            <table class="table table-hover border-1 border">
              ...
              <tbody>
                <tr
                  v-for="product in managementData?.products"
                  :key="product.id"
                >
    ...
    <div
      class="row align-items-center border border-1 border-secondary m-4 rounded rounded-2 p-4"
    >
      <div class="col">
        <h1>Download Zip File</h1>
        <p class="text-secondary p-0 m-0">
          Download Zip File containing csv files with engagement data
        </p>
      </div>
      <div class="col">
        <button
    ...
    <div class="img-container">
      <div class="img-div">
        <img
          :src="boughtProductGraph"
          alt="Graph showing frequency at which products are bougth"
        />
        <p>
          This Graph shows the frequency at which each product is bought by user
        </p>
      </div>
      <div>
        <img
          :src="couponGraph"
          alt="Graph showing frequency at which coupons are used"
        />
        <p>
          This Graph shows the frequency at which each coupon is used by user
        </p>
      </div>
      <div>
        <img
          :src="favProductGraph"
          alt="Graph showing frequency at which products are wishlisted by users"
        />
        <p>
          This Graph shows the frequency at which each product is wishlisted by
          user
        </p>
      </div>
    </div>
    <teleport to="#modal-root">
      <UserProfileUpdateForm
        v-show="isProfileUpdateForm"
        @close="showProfileUpdateForm"
      />
      <UpdatePasswordForm
        v-show="isUpdatePasswordFormshown"
        @close="showUpdatePasswordForm"
      />
    </teleport>
    <!-- <p>{{ managementData }}</p> -->
  </div>
